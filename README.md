...
![Intro](docs/images/ms-civica-card_intro.png "Intro")
# Table of Contents
  * [Project Structure](#structure)
  * [FrontEnd](#frontend)
    *  [Environment variables](#frontend_env_vars)
  * [API](#api)
    * [GraphQL throught sales-gateway API](#api_sales-gateway_graphql)
  * [BackEnd](#backend)
    *  [civica-card](#backend_civica-card)
        * [Environment variables](#backend_civica-card_env_vars)
        * [Event Sourcing](#backend_civica-card_eventsourcing)
        * [CronJobs](#backend_civica-card_cronjobs)
...
├── api                                 => Micro-APIs  
│   └── sales-gateway                           => Micro-API for [sales-gateway API](https://github.com/nebulae-pyxis/sales-gateway)  
├── backend                             => Micro-BackEnds  
│   ├── civica-card                     => Micro-BackEnd responsible for ...
...
## GraphQL throught sales-gateway API <a name="api_sales-gateway_graphql"></a>
These are the exposed GraphQL functions throught the [sales-gateway API](https://github.com/nebulae-pyxis/sales-gateway).  

Note: You may find the GraphQL schema [here](api/sales-gateway/graphql/civica-card/schema.gql)
...
#### civica-cardHelloWorldSubscription
...
## civica-card <a name="backend_civica-card"></a>
...


### Environment variables <a name="backend_civica-card_env_vars"></a>
...
```
#### Notes: 
  * ENV VARS for development are [here](backend/civica-card/.env)
  * ENV VARS for production are [here](deployment/gke/deployment-civica-card.yaml)

### Event Sourcing <a name="backend_civica-card_eventsourcing"></a>
...
### CronJobs <a name="backend_civica-card_cronjobs"></a>
...
### 1. clone this repo  
   ```git clone https://github.com/nebulae-pyxis/ms-civica-card.git```  
...
### 5. Compose FrontEnd
```
nebulae compose-ui development --shell-type=FUSE2_ANGULAR --shell-repo=https://github.com/nebulae-pyxis/emi --frontend-id=emi --output-dir=/FULL_PATH_TO_REPO/ms-civica-card/playground/emi  --setup-file=/FULL_PATH_TO_REPO/ms-civica-card/etc/mfe-setup.json
```

### 6. Compose the API sales-gateway
```
nebulae compose-api development --api-type=NEBULAE_GATEWAY --api-repo=https://github.com/nebulae-pyxis/sales-gateway --api-id=sales-gateway --output-dir=FULL_PATH_TO_REPO/ms-civica-card/playground/sales-gateway  --setup-file=FULL_PATH_TO_REPO/ms-civica-card/etc/mapi-setup.json
```
...
* set this key value to the **JWT_PUBLIC_KEY** atribute in the following files: *WORKING_FOLDER*/ms-civica-card/backend/civica-card/.env   *WORKING_FOLDER*/ms-civica-card/playground/sales-gateway/.env  
...
* Setup the Apollo engine key to trace API requests
    * create a key at https://engine.apollographql.com/ and set it to the **APOLLO_ENGINE_API_KEY** atribute in the playground/sales-gateway/.env file
...
```
cd backend/civica-card/
npm install
npm start
```
3. Start the API sales-gateway
```
cd playground/sales-gateway
